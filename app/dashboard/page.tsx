      ...
      {viewMode === "list" && !loading && filtered.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {filtered.map(habit => {
            ...
            return (
              <div key={habit.id}>
                <div className="row-hover" onClick={() => setExpandedHabit(isExpanded ? null : habit.id)}
                  ...
                  <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
                    ...
                    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}
                      onClick={(e) => { e.stopPropagation(); handleCheckin(habit.id); }}>
                      {done ? (
                  ...
                  <span className="mono" style={{ fontSize: 9, color: "var(--faint)", transition: "transform 150ms ease", transform: isExpanded ? "rotate(180deg)" : "none" }}>▾</span>
                </div>
                {/* Expanded detail */}
                {isExpanded && (
                  <div style={{ marginLeft: 48, marginBottom: 8, padding: "12px 16px", borderRadius: 8, background: "var(--surface)" }}>
                    ...
                    <div style={{ display: "flex", gap: 3, marginBottom: 12, alignItems: "center" }}>
                      {last14.map(d => (
                        <div key={d.date} title={d.date} style={{
                          ...
                          background: d.done ? "var(--ink)" : d.partial ? "var(--hairline2)" : "var(--hairline)",
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button onClick={(e) => { e.stopPropagation(); setExpandedHabit(null); /* open edit */ }} style={{
                        padding: "4px 10px", borderRadius: 6, border: "1px solid var(--hairline)",
                        background: "transparent", color: "var(--mute)", fontSize: 12, cursor: "pointer", fontFamily: "inherit",
                      }}>Editar</button>
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(habit.id); }} style={{
                        padding: "4px 10px", borderRadius: 6, border: "1px solid var(--hairline)",
                        background: "transparent", color: "var(--faint)", fontSize: 12, cursor: "pointer", fontFamily: "inherit",
                      }}>Eliminar</button>
                    </div>
                  </div>
                )}
        ...
        <div style={{ background: "var(--surface)", borderRadius: 10, border: "1px solid var(--hairline)", padding: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3 }}>
            ...
            {monthDays.map(day => {
              ...
              const empty = filtered.filter(h => !checkins[h.id]?.[d]);
              const pct = filtered.length > 0 ? completed.length / filtered.length : 0;
              return (
                <button key={d} onClick={() => setSelectedDay(isSelected ? null : d)} style={{
                  padding: "6px 3px", borderRadius: 8, textAlign: "center",
                  border: "none", cursor: "pointer", fontFamily: "inherit",
                  background: isSelected ? "var(--whisper)" : "transparent",
                  ...
                  {filtered.length > 0 && (
                    <div style={{ display: "flex", justifyContent: "center", gap: 1.5, marginTop: 3, flexWrap: "wrap", maxWidth: "100%", overflow: "hidden" }}>
                      {completed.slice(0, 5).map(h => (
                        <div key={h.id} style={{ width: 4, height: 4, borderRadius: "50%", background: "var(--ink)", flexShrink: 0 }} />
                      ))}
                      {partial.slice(0, 3).map(h => (
                        <div key={h.id} style={{ width: 4, height: 4, borderRadius: "50%", background: "var(--hairline2)", flexShrink: 0 }} />
                      ))}
          ...
          {selectedDay && (
            <div style={{ marginTop: 16, padding: "12px 16px", borderRadius: 8, background: "var(--whisper)" }}>
              <div className="mono" style={{ fontSize: 10, color: "var(--mute)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>
                {formatDate(new Date(selectedDay + "T00:00:00"), "EEEE d 'de' MMMM", { locale: es }).replace(/^\w/, c => c.toUpperCase())}
              </div>
              {filtered.filter(h => checkins[h.id]?.[selectedDay]).length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {filtered.map(h => {
                    const c = checkins[h.id]?.[selectedDay] || 0;
                    const t = h.targetPerDay || 1;
                    const done = c >= t;
                    return (
                      <div key={h.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                        <HabitIcon name={h.icon || "star"} size={12} color={done ? "var(--ink)" : "var(--faint)"} />
                        <span style={{ color: done ? "var(--ink)" : "var(--mute)", flex: 1 }}>{h.title}</span>
                        <span className="mono" style={{ fontSize: 10, color: done ? "var(--ink)" : "var(--faint)" }}>
                          {c}/{t}
                        </span>
                      </div>
                    );
