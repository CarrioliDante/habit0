...
interface HabitFormProps {
  habit?: Habit;
  onSubmit: (data: { title: string; description?: string; cadence: string; targetPerDay: number }) => void;
  onCancel: () => void;
  loading?: boolean;
}
...
export function HabitForm({ habit, onSubmit, onCancel, loading }: HabitFormProps) {
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ title: title.trim(), description: description.trim() || undefined, cadence, targetPerDay: target });
  };
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <select value={cadence} onChange={e => setCadence(e.target.value)} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <input
                ...
                style={{
                  width: 48, padding: "10px 8px", borderRadius: 8,
                  border: "1px solid var(--hairline)", background: "var(--bg)",
                  fontSize: 14, color: "var(--ink)", outline: "none",
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
          ...
          <button onClick={handleSubmit} disabled={loading || !title.trim()} style={{
            padding: "8px 16px", borderRadius: 8,
            border: "none", background: "var(--ink)", color: "var(--inverse)",
            fontSize: 13, fontWeight: 500, cursor: "pointer",
            fontFamily: "inherit", opacity: loading ? 0.5 : 1,
          }}>{loading ? "Guardando..." : habit ? "Guardar" : "Crear"}</button>
        </div>
      </div>
    </div>
  );
